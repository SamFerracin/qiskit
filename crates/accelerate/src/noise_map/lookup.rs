...
// The lookup tables are pretty illegible if we have all the syntactic noise of `BitTermSam`.
use super::BitTermSam::{self, *};
...
/// The allowable items of `BitTermSam`.  This is used by the lookup expansion; we need a const-safe
/// way of iterating through all of the variants.
static BIT_TERMS: [BitTermSam; 9] = [X, Plus, Minus, Y, Right, Left, Z, Zero, One];

/// Create a full binary lookup table for all valid combinations of `BitTermSam`.
...
expand_lookup_binary!(
    static MATMUL: [Option<&'static [(Complex64, BitTermSam)]>],
    matmul_generate,
    None
);
/// Calculate the matrix multiplication of two [BitTermSam]s.
///
/// This is done by a static lookup table.
///
/// In the output, `None` means that the result was zero.  Beyond that, the slice should be
/// interpreted as a sum of the coefficient multiplied by the [BitTermSam] pairs.  An empty slice means
/// the identity.
#[inline(always)]
pub fn matmul(left: BitTermSam, right: BitTermSam) -> Option<&'static [(Complex64, BitTermSam)]> {
...
const fn matmul_generate(left: BitTermSam, right: BitTermSam) -> Option<&'static [(Complex64, BitTermSam)]> {
