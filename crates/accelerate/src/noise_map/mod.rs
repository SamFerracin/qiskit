...
/// not allowing the stored values to be outside the valid `BitTermSam`s, but doing so in Python would
/// make it very difficult to use the class efficiently with Numpy array views.  We attach this
/// sister class of `BitTermSam` to `NoiseMapSam` as a scoped class.
///
/// # Representation
///
...
///
/// This operator does not store phase terms of $-i$.  `BitTermSam::Y` has `(1, 1)` as its `(z, x)`
...
/// `BitTermSam`s.  For interop with Python space, we accept Numpy arrays of `u8` to represent this,
/// which we transmute into slices of `BitTermSam`, after checking that all the values are correct (or
/// skipping the check if Python space promises that it upheld the checks).
///
/// We deliberately _don't_ impl `numpy::Element` for `BitTermSam` (which would let us accept and
/// return `PyArray1<BitTermSam>` at Python-space boundaries) so that it's clear when we're doing
...
impl From<BitTermSam> for u8 {
    fn from(value: BitTermSam) -> u8 {
        value as u8
    }
}
unsafe impl ::bytemuck::CheckedBitPattern for BitTermSam {
    type Bits = u8;
...
unsafe impl ::bytemuck::NoUninit for BitTermSam {}

impl BitTermSam {
    /// Get the label of this `BitTermSam` used in Python-space applications.  This is a single-letter
    /// string.
    #[inline]
    ...
    /// Get the name of this `BitTermSam`, which is how Python space refers to the integer constant.
    #[inline]
    ...
    /// Attempt to convert a `u8` into `BitTermSam`.
    ///
    ...
    fn try_from_u8(value: u8) -> Result<Option<Self>, BitTermFromU8Error> {
        match value {
            b'+' => Ok(Some(BitTermSam::Plus)),
            b'-' => Ok(Some(BitTermSam::Minus)),
            b'0' => Ok(Some(BitTermSam::Zero)),
            b'1' => Ok(Some(BitTermSam::One)),
            b'I' => Ok(None),
            b'X' => Ok(Some(BitTermSam::X)),
            b'Y' => Ok(Some(BitTermSam::Y)),
            b'Z' => Ok(Some(BitTermSam::Z)),
            b'l' => Ok(Some(BitTermSam::Left)),
            b'r' => Ok(Some(BitTermSam::Right)),
    ...
    pub fn is_projector(&self) -> bool {
        !matches!(self, BitTermSam::X | BitTermSam::Y | BitTermSam::Z)
    }
}

fn bit_term_as_pauli(bit: &BitTermSam) -> &'static [(bool, Option<BitTermSam>)] {
    match bit {
        BitTermSam::X => &[(true, Some(BitTermSam::X))],
        BitTermSam::Y => &[(true, Some(BitTermSam::Y))],
        BitTermSam::Z => &[(true, Some(BitTermSam::Z))],
        BitTermSam::Plus => &[(true, None), (true, Some(BitTermSam::X))],
        BitTermSam::Minus => &[(true, None), (false, Some(BitTermSam::X))],
        BitTermSam::Right => &[(true, None), (true, Some(BitTermSam::Y))],
        BitTermSam::Left => &[(true, None), (false, Some(BitTermSam::Y))],
        BitTermSam::Zero => &[(true, None), (true, Some(BitTermSam::Z))],
        BitTermSam::One => &[(true, None), (false, Some(BitTermSam::Z))],
    }
}

/// The error type for a failed conversion into `BitTermSam`.
#[derive(Error, Debug)]
#[error("{0} is not a valid letter of the single-qubit alphabet")]
pub struct BitTermFromU8Error(u8);

// `BitTermSam` allows safe `as` casting into `u8`.  This is the reverse, which is fallible, because
// `BitTermSam` is a value-wise subtype of `u8`.
impl ::std::convert::TryFrom<u8> for BitTermSam {
...
/// Error cases stemming from data coherence at the point of entry into `NoiseMapSam` from
...
///
/// See [PyNoiseMapSam] for detailed docs.
#[derive(Clone, Debug, PartialEq)]
pub struct NoiseMapSam {
...
impl NoiseMapSam {
    ...
    ///
    /// Recall that two [NoiseMapSam]s that have different term orders can still represent the
    /// same object.  Use [canonicalize] to apply a canonical ordering to the terms.
    pub fn iter(&'_ self) -> impl ExactSizeIterator<Item = SparseTermViewSam<'_>> + '_ {
    ...
    /// Get the indices of each [BitTermSam].
    #[inline]
    ...
    /// # Safety
    ///
    /// Modifying the indices can cause an incoherent state of the [NoiseMapSam].
    ...
    /// # Safety
    ///
    /// Modifying the boundaries can cause an incoherent state of the [NoiseMapSam].
    ...
    /// Get the [BitTermSam]s in the observable.
    #[inline]
    pub fn bit_terms(&self) -> &[BitTermSam] {
        &self.bit_terms
    }

    /// Get a muitable slice of the bit terms.
    #[inline]
    pub fn bit_terms_mut(&mut self) -> &mut [BitTermSam] {
        &mut self.bit_terms
    }
    ...
    pub fn canonicalize(&self, tol: f64) -> NoiseMapSam {
        ...
        let mut out = NoiseMapSam::zero(self.num_qubits);
    ...
    pub fn adjoint(&self) -> NoiseMapSam {
        NoiseMapSam {
            num_qubits: self.num_qubits,
    ...
    /// on the coefficients.
    pub fn transpose(&self) -> NoiseMapSam {
        ...
        for term in out.iter_mut() {
            for bit_term in term.bit_terms {
                match bit_term {
                    ...
                    BitTermSam::Right => {
                        *bit_term = BitTermSam::Left;
                    }
                    BitTermSam::Left => {
                        *bit_term = BitTermSam::Right;
                    }
                    _ => (),
                }
            }
    ...
    pub fn conjugate(&self) -> NoiseMapSam {
    ...
    /// Compose another [NoiseMapSam] onto this one.
    ///
    ...
    /// observable, as the alphabet of [NoiseMapSam] is not closed under composition; the
    ...
    pub fn compose(&self, other: &NoiseMapSam) -> NoiseMapSam {
    ...
    /// Compose another [NoiseMapSam] onto this one, remapping the qubits.
    ///
    ...
    pub fn compose_map(
        ...
        let mut out = NoiseMapSam::zero(self.num_qubits);
        let mut mapped_term = btree_map::BTreeMap::<u32, BitTermSam>::new();
        let mut term_state = compose::Iter::new(self.num_qubits);
    ...
    ///
    /// This is effectively an indexing operation into the [NoiseMapSam].  Recall that two
    /// [NoiseMapSam]s that have different term orders can still represent the same object.
    /// Use [canonicalize] to apply a canonical ordering to the terms.
    ///
    /// # Panics
    ///
    /// If the index is out of bounds.
    pub fn term(&self, index: usize) -> SparseTermViewSam {
    ...
    pub fn as_paulis(&self) -> Self {
        let mut paulis: Vec<BitTermSam> = Vec::new(); // maybe get capacity here
    ...
    pub fn add_dense_label<L: AsRef<[u8]>>(
        ...
        for (i, letter) in label.iter().rev().enumerate() {
            match BitTermSam::try_from_u8(*letter) {
                Ok(Some(term)) => {
    ...
    pub fn add_term(&mut self, term: SparseTermViewSam) -> Result<(), ArithmeticErrorSam> {
        if self.num_qubits != term.num_qubits {
            return Err(ArithmeticErrorSam::MismatchedQubits {
    ...
    pub fn check_equal_qubits(&self, other: &NoiseMapSam) -> Result<(), ArithmeticErrorSam> {
        if self.num_qubits != other.num_qubits {
            Err(ArithmeticErrorSam::MismatchedQubits {
                left: self.num_qubits,
...
impl ::std::ops::Add<&NoiseMapSam> for NoiseMapSam {
    type Output = NoiseMapSam;

    fn add(mut self, rhs: &NoiseMapSam) -> NoiseMapSam {
        self += rhs;
        self
    }
}
impl ::std::ops::Add for &NoiseMapSam {
    type Output = NoiseMapSam;

    fn add(self, rhs: &NoiseMapSam) -> NoiseMapSam {
        let mut out = NoiseMapSam::with_capacity(
            self.num_qubits,
...
impl ::std::ops::AddAssign<&NoiseMapSam> for NoiseMapSam {
    fn add_assign(&mut self, rhs: &NoiseMapSam) {
...
impl ::std::ops::Sub<&NoiseMapSam> for NoiseMapSam {
    type Output = NoiseMapSam;

    fn sub(mut self, rhs: &NoiseMapSam) -> NoiseMapSam {
        self -= rhs;
        self
    }
}
impl ::std::ops::Sub for &NoiseMapSam {
    type Output = NoiseMapSam;

    fn sub(self, rhs: &NoiseMapSam) -> NoiseMapSam {
        let mut out = NoiseMapSam::with_capacity(
            self.num_qubits,
            self.coeffs.len() + rhs.coeffs.len(),
            self.bit_terms.len() + rhs.bit_terms.len(),
...
impl ::std::ops::SubAssign<&NoiseMapSam> for NoiseMapSam {
    fn sub_assign(&mut self, rhs: &NoiseMapSam) {
        if self.num_qubits != rhs.num_qubits {
...
impl ::std::ops::Mul<Complex64> for NoiseMapSam {
    type Output = NoiseMapSam;

    fn mul(mut self, rhs: Complex64) -> NoiseMapSam {
        self *= rhs;
        self
    }
}
impl ::std::ops::Mul<Complex64> for &NoiseMapSam {
    ...
    fn mul(self, rhs: Complex64) -> NoiseMapSam {
        if rhs == Complex64::new(0.0, 0.0) {
            NoiseMapSam::zero(self.num_qubits)
        } else {
            NoiseMapSam {
                num_qubits: self.num_qubits,
...
impl ::std::ops::Mul<NoiseMapSam> for Complex64 {
    type Output = NoiseMapSam;

    fn mul(self, mut rhs: NoiseMapSam) -> NoiseMapSam {
        rhs *= self;
        rhs
    }
}
impl ::std::ops::Mul<&NoiseMapSam> for Complex64 {
    ...
    fn mul(self, rhs: &NoiseMapSam) -> NoiseMapSam {
        rhs * self
    }
}
impl ::std::ops::MulAssign<Complex64> for NoiseMapSam {
...
impl ::std::ops::Div<Complex64> for NoiseMapSam {
    type Output = NoiseMapSam;

    fn div(mut self, rhs: Complex64) -> NoiseMapSam {
        self /= rhs;
        self
    }
}
impl ::std::ops::Div<Complex64> for &NoiseMapSam {
    ...
    fn div(self, rhs: Complex64) -> NoiseMapSam {
        NoiseMapSam {
            num_qubits: self.num_qubits,
            coeffs: self.coeffs.iter().map(|c| c / rhs).collect(),
            bit_terms: self.bit_terms.clone(),
...
impl ::std::ops::DivAssign<Complex64> for NoiseMapSam {
    fn div_assign(&mut self, rhs: Complex64) {
        self.coeffs.iter_mut().for_each(|c| *c /= rhs)
    }
}

impl ::std::ops::Neg for &NoiseMapSam {
    type Output = NoiseMapSam;

    fn neg(self) -> NoiseMapSam {
        NoiseMapSam {
            num_qubits: self.num_qubits,
...
impl ::std::ops::Neg for NoiseMapSam {
    ...
    fn neg(mut self) -> NoiseMapSam {
        self.coeffs.iter_mut().for_each(|c| *c = -*c);
        self
    }
}

/// Worker objects for the [NoiseMapSam::compose]-alike functions.
mod compose {
    use super::*;
    ...
    struct MultipleItem {
        ...
        slice: &'static [(Complex64, BitTermSam)],
    }

    /// An implementation of the multiple Cartesian-product iterator that produces the sum terms
    /// that make up the composition of two individual [NoiseMapSam] terms.
    ///
    ...
    /// buffers to the output [NoiseMapSam] each time anyway).
    pub struct Iter {
        num_qubits: u32,
        ...
        /// out of.
        indices: Vec<u32>,
        /// The full set of [BitTermSam]s (including ones that don't correspond to multiples).  The
    ...
    impl Iter {
        ...
        pub fn load_from(
            &mut self,
            coeff: Complex64,
            left: impl IntoIterator<Item = (u32, BitTermSam)>,
            right: impl IntoIterator<Item = (u32, BitTermSam)>,
        ) {
        ...
        fn iter_item(&self) -> SparseTermViewSam {
            SparseTermViewSam {
                num_qubits: self.num_qubits,
        ...
        pub fn next(&mut self) -> Option<SparseTermViewSam> {
            if self.exhausted {
                return None;
            }
            if !self.started {
...
/// A view object onto a single term of a `NoiseMapSam`.
///
...
pub struct SparseTermViewSam<'a> {
    pub num_qubits: u32,
    pub coeff: Complex64,
    pub bit_terms: &'a [BitTermSam],
    pub indices: &'a [u32],
}
impl SparseTermViewSam<'_> {
    /// Convert this `SparseTermViewSam` into an owning [SparseTerm] of the same data.
...
/// A mutable view object onto a single term of a [NoiseMapSam].
///
/// The lengths of [bit_terms] and [indices] are guaranteed to be created equal, but might be zero
...
pub struct SparseTermViewMut<'a> {
    ...
    pub bit_terms: &'a mut [BitTermSam],
    pub indices: &'a [u32],
}

/// Iterator type allowing in-place mutation of the [NoiseMapSam].
///
/// Created by [NoiseMapSam::iter_mut].
#[derive(Debug)]
pub struct IterMut<'a> {
    num_qubits: u32,
    coeffs: &'a mut [Complex64],
    bit_terms: &'a mut [BitTermSam],
    indices: &'a [u32],
    boundaries: &'a [usize],
    i: usize,
}
impl<'a> From<&'a mut NoiseMapSam> for IterMut<'a> {
    fn from(value: &mut NoiseMapSam) -> IterMut {
        IterMut {
...
/// A single term from a complete :class:`NoiseMapSam`.
///
/// These are typically created by indexing into or iterating through a :class:`NoiseMapSam`.
#[derive(Clone, Debug, PartialEq)]
...
impl SparseTerm {
    ...
    pub fn view(&self) -> SparseTermViewSam {
        SparseTermViewSam {
            num_qubits: self.num_qubits,
            coeff: self.coeff,
            bit_terms: &self.bit_terms,
    ...
    /// Convert this term to a complete :class:`NoiseMapSam`.
    pub fn to_observable(&self) -> NoiseMapSam {
        NoiseMapSam {
            num_qubits: self.num_qubits,
...
impl From<ArithmeticErrorSam> for PyErr {
    fn from(value: ArithmeticErrorSam) -> PyErr {
        PyValueError::new_err(value.to_string())
    }
}

/// The single-character string label used to represent this term in the :class:`NoiseMapSam`
/// alphabet.
#[pyfunction]
#[pyo3(name = "label")]
fn bit_term_label(py: Python, slf: BitTermSam) -> &Bound<PyString> {
    // This doesn't use `py_label` so we can use `intern!`.
    match slf {
        BitTermSam::X => intern!(py, "X"),
        BitTermSam::Plus => intern!(py, "+"),
        BitTermSam::Minus => intern!(py, "-"),
        BitTermSam::Y => intern!(py, "Y"),
        BitTermSam::Right => intern!(py, "r"),
        BitTermSam::Left => intern!(py, "l"),
        BitTermSam::Z => intern!(py, "Z"),
        BitTermSam::Zero => intern!(py, "0"),
        BitTermSam::One => intern!(py, "1"),
    }
}
/// Construct the Python-space `IntEnum` that represents the same values as the Rust-spce `BitTermSam`.
///
/// We don't make `BitTermSam` a direct `pyclass` because we want the behaviour of `IntEnum`, which
...
///
/// The resulting class is attached to `NoiseMapSam` as a class attribute, and its
    ...
    let obj = py.import("enum")?.getattr("IntEnum")?.call(
        ...
        Some(
            &[
                ...
                ("qualname", "NoiseMapSam.BitTermSam"),
            ]
            .into_py_dict(py)?,
        ),
    )?;
...
// efficiently.
impl<'py> IntoPyObject<'py> for BitTermSam {
    type Target = PyAny;
                ...
                ::bytemuck::checked::try_cast(val as u8)
                    ...
                    .map(|term: BitTermSam| {
                        py_enum
                            ...
                            .expect("the created `BitTermSam` enum should have matching attribute names to the terms")
                            .unbind()
                    })
            })
        });
...
impl<'py> FromPyObject<'py> for BitTermSam {
    fn extract_bound(ob: &Bound<'py, PyAny>) -> PyResult<Self> {
        let value = ob
            .extract::<isize>()
            .map_err(|_| match ob.get_type().repr() {
                Ok(repr) => PyTypeError::new_err(format!("bad type for 'BitTermSam': {}", repr)),
                Err(err) => err,
            })?;
        let value_error = || {
            PyValueError::new_err(format!(
                "value {} is not a valid letter of the single-qubit alphabet for 'BitTermSam'",
                value
            ))
        };
...
#[pymethods]
impl PySparseTerm {
    // Mark the Python class as being defined "within" the `NoiseMapSam` class namespace.
    #[classattr]
    ...
    /// Convert this term to a complete :class:`NoiseMapSam`.
    fn to_observable(&self) -> PyResult<PyNoiseMapSam> {
        let obs = NoiseMapSam::new(
    ...
    fn get_bit_terms(slf_: Bound<Self>) -> Bound<PyArray1<u8>> {
        ...
        // `Box<[BitTermSam]>` allocation (including dropping or reallocating it) other than the entire
...
/// .. math::
///
///     \text{\texttt{NoiseMapSam}} = \sum_i c_i \bigotimes_n A^{(n)}_i
///
...
/// .. table:: Alphabet of single-qubit terms used in :class:`NoiseMapSam`
///
///   =======  =======================================  ===============  ===========================
///   Label    Operator                                 Numeric value    :class:`.BitTermSam` attribute
///   =======  =======================================  ===============  ===========================
///   ``"I"``  :math:`I` (identity)                     Not stored.      Not stored.
///
///   ``"X"``  :math:`X` (Pauli X)                      ``0b0010`` (2)   :attr:`~.BitTermSam.X`
///
///   ``"Y"``  :math:`Y` (Pauli Y)                      ``0b0011`` (3)   :attr:`~.BitTermSam.Y`
///
///   ``"Z"``  :math:`Z` (Pauli Z)                      ``0b0001`` (1)   :attr:`~.BitTermSam.Z`
///
///   ``"+"``  :math:`\lvert+\rangle\langle+\rvert`     ``0b1010`` (10)  :attr:`~.BitTermSam.PLUS`
///            (projector to positive eigenstate of X)
///
///   ``"-"``  :math:`\lvert-\rangle\langle-\rvert`     ``0b0110`` (6)   :attr:`~.BitTermSam.MINUS`
///            (projector to negative eigenstate of X)
///
///   ``"r"``  :math:`\lvert r\rangle\langle r\rvert`   ``0b1011`` (11)  :attr:`~.BitTermSam.RIGHT`
///            (projector to positive eigenstate of Y)
///
///   ``"l"``  :math:`\lvert l\rangle\langle l\rvert`   ``0b0111`` (7)   :attr:`~.BitTermSam.LEFT`
///            (projector to negative eigenstate of Y)
///
///   ``"0"``  :math:`\lvert0\rangle\langle0\rvert`     ``0b1001`` (9)   :attr:`~.BitTermSam.ZERO`
///            (projector to positive eigenstate of Z)
///
///   ``"1"``  :math:`\lvert1\rangle\langle1\rvert`     ``0b0101`` (5)   :attr:`~.BitTermSam.ONE`
...
///
/// :class:`NoiseMapSam` uses its particular overcomplete basis with the aim of making
...
/// terms.  :class:`NoiseMapSam` requires only a single term to store this.
///
/// The downside to this is that it is impractical to take an arbitrary matrix or
/// :class:`.SparsePauliOp` and find the *best* :class:`NoiseMapSam` representation.  You
/// typically will want to construct a :class:`NoiseMapSam` directly, rather than trying to
/// decompose into one.
///
///
/// Representation
/// ==============
///
/// The internal representation of a :class:`NoiseMapSam` stores only the non-identity qubit
/// operators.  This makes it significantly more efficient to represent observables such as
/// :math:`\sum_{n\in \text{qubits}} Z^{(n)}`; :class:`NoiseMapSam` requires an amount of
...
/// .. table:: Data arrays used to represent :class:`.NoiseMapSam`
///
///   ==================  ===========  =============================================================
...
///                                    in :attr:`bit_terms`.  :class:`NoiseMapSam` requires
...
///   :attr:`coeffs` is ``[1.0, -1.0]``, :attr:`bit_terms` is ``[BitTermSam.Z, BitTermSam.Z, BitTermSam.Y,
///   BitTermSam.X]`` and :attr:`indices` is ``[0, 2, 1, 3]``.  The operator might act on more than
///   four qubits, depending on the :attr:`num_qubits` parameter.  The :attr:`bit_terms` are integer
///   values, whose magic numbers can be accessed via the :class:`BitTermSam` attribute class.  Note
...
///     value.
///
/// .. py:class:: NoiseMapSam.BitTermSam
///
...
///
///     This class is attached to :class:`.NoiseMapSam`.  Access it as
///     :class:`.NoiseMapSam.BitTermSam`.  If this is too much typing, and you are solely dealing
///     with :class:¬NoiseMapSam` objects and the :class:`BitTermSam` name is not ambiguous, you
///     might want to shorten it as::
///
///         >>> ops = NoiseMapSam.BitTermSam
///         >>> assert ops.X is NoiseMapSam.BitTermSam.X
///
...
///
///         >>> assert NoiseMapSam.BitTermSam.ZERO is NoiseMapSam.BitTermSam["0"]
///
...
///     ------
///
///     .. autoattribute:: qiskit.quantum_info::NoiseMapSam.BitTermSam.X
///
///         The Pauli :math:`X` operator.  Uses the single-letter label ``"X"``.
///
///     .. autoattribute:: qiskit.quantum_info::NoiseMapSam.BitTermSam.PLUS
///
...
///
///     .. autoattribute:: qiskit.quantum_info::NoiseMapSam.BitTermSam.MINUS
///
///         The projector to the negative eigenstate of the :math:`X` operator:
///         :math:`\lvert-\rangle\langle-\rvert`.  Uses the single-letter label ``"-"``.
///
///     .. autoattribute:: qiskit.quantum_info::NoiseMapSam.BitTermSam.Y
///
///         The Pauli :math:`Y` operator.  Uses the single-letter label ``"Y"``.
///
///     .. autoattribute:: qiskit.quantum_info::NoiseMapSam.BitTermSam.RIGHT
///
...
///
///     .. autoattribute:: qiskit.quantum_info::NoiseMapSam.BitTermSam.LEFT
///
...
///
///     .. autoattribute:: qiskit.quantum_info::NoiseMapSam.BitTermSam.Z
///
///         The Pauli :math:`Z` operator.  Uses the single-letter label ``"Z"``.
///
///     .. autoattribute:: qiskit.quantum_info::NoiseMapSam.BitTermSam.ZERO
///
...
///
///     .. autoattribute:: qiskit.quantum_info::NoiseMapSam.BitTermSam.ONE
///
...
///     ----------
///
///     .. autoproperty:: qiskit.quantum_info::NoiseMapSam.BitTermSam.label
///
...
/// this.  For example::
///
///     >>> obs = NoiseMapSam.from_list([("XZY", 1.5j), ("+1r", -0.5)])
...
///     >>> assert obs == NoiseMapSam.from_list([("XZY", 1.5j), ("XZY", -0.5)])
///
/// .. note::
///
...
/// :class:`NoiseMapSam`.  For example, the same set of single-bit terms and their
...
/// of the sum in a :class:`NoiseMapSam` can be in any order while representing the same
/// observable, since addition is commutative (although while floating-point addition is not
/// associative, :class:`NoiseMapSam` makes no guarantees about the summation order).
///
...
/// structurally.
///
/// You can put a :class:`NoiseMapSam` in canonical form by using the :meth:`simplify` method.
...
///             return (left - right).simplify(tol) == NoiseMapSam.zero(left.num_qubits)
///
/// .. note::
///
...
/// --------
///
/// :class:`NoiseMapSam` behaves as `a Python sequence
...
///
/// .. autoclass:: qiskit.quantum_info::NoiseMapSam.Term
///     :members:
///
/// Construction
/// ============
///
/// :class:`NoiseMapSam` defines several constructors.  The default constructor will attempt to
...
///                                 :class:`.NoiseMapSam`.
///
///   :meth:`from_sparse_pauli_op`  Raise a :class:`.SparsePauliOp` into a :class:`NoiseMapSam`.
///
...
///   ============================  ================================================================
///
/// .. py:function:: NoiseMapSam.__new__(data, /, num_qubits=None)
///
///     The default constructor of :class:`NoiseMapSam`.
///
...
///
///     :param data: The data type of the input.  This can be another :class:`NoiseMapSam`, in
...
/// ===========
///
/// An existing :class:`NoiseMapSam` can be converted into other :mod:`~qiskit.quantum_info`
/// operators or generic formats.  Beware that other objects may not be able to represent the same
/// observable as efficiently as :class:`NoiseMapSam`, including potentially needed
...
///   Method                       Summary
///   ===========================  =================================================================
///   :meth:`as_paulis`            Create a new :class:`NoiseMapSam`, expanding in terms
...
/// =========================
///
/// :class:`NoiseMapSam` supports the standard set of Python mathematical operators like other
...
///
/// A :class:`NoiseMapSam` has a well-defined :meth:`adjoint`.  The notions of scalar complex
...
/// ----------------
///
/// Internally, :class:`NoiseMapSam` is in-place mutable, including using over-allocating
...
#[pyclass(name = "NoiseMapSam", module = "qiskit.quantum_info", sequence)]
#[derive(Debug)]
pub struct PyNoiseMapSam {
    // This class keeps a pointer to a pure Rust-SparseTerm and serves as interface from Python.
    inner: Arc<RwLock<NoiseMapSam>>,
}
#[pymethods]
impl PyNoiseMapSam {
    ...
    fn py_new(data: &Bound<PyAny>, num_qubits: Option<u32>) -> PyResult<Self> {
        ...
        Err(PyTypeError::new_err(format!(
            "unknown input format for 'NoiseMapSam': {}",
            data.get_type().repr()?,
        )))
    }
    ...
    ///     .. code-block:: python
    ///
    ///         >>> obs = NoiseMapSam.from_list([("IXZ+lr01", 2.5), ("ZXI-rl10", 0.5j)])
    ///         >>> assert obs == obs.copy()
    ...
    /// It has no terms.  It is the identity element for addition of two :class:`NoiseMapSam`
    /// instances; anything added to the zero operator is equal to itself.
    ///
    /// If you want the projector onto the all zeros state, use::
    ///
    ///     >>> num_qubits = 10
    ///     >>> all_zeros = NoiseMapSam.from_label("0" * num_qubits)
    ///
    /// Examples:
    ///
    ///     Get the zero operator for 100 qubits::
    ///
    ///         >>> NoiseMapSam.zero(100)
    ///         <NoiseMapSam with 0 terms on 100 qubits: 0.0>
    #[pyo3(signature = (/, num_qubits))]
    #[staticmethod]
    pub fn zero(num_qubits: u32) -> Self {
        NoiseMapSam::zero(num_qubits).into()
    }
    ...
    ///
    ///         >>> NoiseMapSam.identity(100)
    ///         <NoiseMapSam with 1 term on 100 qubits: (1+0j)()>
    #[pyo3(signature = (/, num_qubits))]
    #[staticmethod]
    pub fn identity(num_qubits: u32) -> Self {
        NoiseMapSam::identity(num_qubits).into()
    }

    /// Construct a :class:`.NoiseMapSam` from a single :class:`.Pauli` instance.
    ///
    ...
    ///         >>> label = "IYXZI"
    ///         >>> pauli = Pauli(label)
    ///         >>> NoiseMapSam.from_pauli(pauli)
    ///         <NoiseMapSam with 1 term on 5 qubits: (1+0j)(Y_3 X_2 Z_1)>
    ///         >>> assert NoiseMapSam.from_label(label) == NoiseMapSam.from_pauli(pauli)
    #[staticmethod]
    ...
    fn from_pauli(pauli: &Bound<PyAny>) -> PyResult<Self> {
        ...
        for (i, (x, z)) in x.as_array().iter().zip(z.as_array().iter()).enumerate() {
            ...
            num_ys += (term == BitTermSam::Y) as isize;
        ...
        let inner = NoiseMapSam::new(num_qubits, coeffs, bit_terms, indices, boundaries)?;
        Ok(inner.into())
    }
    ...
    ///     .. code-block:: python
    ///
    ///         >>> NoiseMapSam.from_label("IIII+ZI")
    ///         <NoiseMapSam with 1 term on 7 qubits: (1+0j)(+_2 Z_1)>
    ///         >>> label = "IYXZI"
    ///         >>> pauli = Pauli(label)
    ///         >>> assert NoiseMapSam.from_label(label) == NoiseMapSam.from_pauli(pauli)
    ///
    ...
    fn from_label(label: &str) -> Result<Self, LabelError> {
        let mut inner = NoiseMapSam::zero(label.len() as u32);
    ...
    /// :ref:`the extended alphabet <sparse-observable-alphabet>` of :class:`.NoiseMapSam`.  In
    ...
    ///
    ///         >>> NoiseMapSam.from_list([
    ///         ...     ("III++", 1.0),
    ///         ...     ("II--I", 1.0j),
    ///         ...     ("I++II", -0.5),
    ///         ...     ("--III", -0.25j),
    ///         ... ])
    ///         <NoiseMapSam with 4 terms on 5 qubits:
    ///             (1+0j)(+_1 +_0) + (0+1j)(-_2 -_1) + (-0.5+0j)(+_3 +_2) + (-0-0.25j)(-_4 -_3)>
    ///
    ///     Use ``num_qubits`` to disambiguate potentially empty inputs::
    ///
    ///         >>> NoiseMapSam.from_list([], num_qubits=10)
    ///         <NoiseMapSam with 0 terms on 10 qubits: 0.0>
    ///
    ...
    ///         >>> coeffs = [1.5j, 2.0, -0.5]
    ///         >>> from_list = NoiseMapSam.from_list(list(zip(labels, coeffs)))
    ///         >>> from_sparse_list = NoiseMapSam.from_sparse_list([
    ///         ...     (label, (3, 2, 1, 0), coeff)
    ...
    fn from_list(iter: Vec<(String, Complex64)>, num_qubits: Option<u32>) -> PyResult<Self> {
        ...
        let mut inner = NoiseMapSam::with_capacity(num_qubits, iter.len(), 0);
    ...
    /// :ref:`the extended alphabet <sparse-observable-alphabet>` of :class:`.NoiseMapSam`.
    ///
    ...
    ///
    ///         >>> NoiseMapSam.from_sparse_list(
    ///         ...     [("ZX", (1, 4), 1.0), ("YY", (0, 3), 2j)],
    ///         ...     num_qubits=5,
    ///         ... )
    ///         <NoiseMapSam with 2 terms on 5 qubits: (1+0j)(X_4 Z_1) + (0+2j)(Y_3 Y_0)>
    ///
    ///     Construct the identity observable (though really, just use :meth:`identity`)::
    ///
    ///         >>> NoiseMapSam.from_sparse_list([("", (), 1.0)], num_qubits=100)
    ///         <NoiseMapSam with 1 term on 100 qubits: (1+0j)()>
    ///
    ...
    fn from_sparse_list(
        ...
        for (label, qubits, _) in iter {
            ...
            for (letter, index) in label.iter().zip(qubits) {
                ...
                entry.insert(
                    BitTermSam::try_from_u8(*letter).map_err(|_| LabelError::OutsideAlphabet)?,
                );
            }
    ...
    /// :meth:`NoiseMapSam.simplify` in addition to obtain a canonical representation.
    ///
    /// .. warning::
    ///
    ...
    ///         >>> obs = NoiseMapSam("+")
    ///         >>> obs.as_paulis()
    ///         <NoiseMapSam with 2 terms on 1 qubit: (0.5+0j)() + (0.5+0j)(X_0)>
    ///         >>> direct = NoiseMapSam.from_list([("I", 0.5), ("Z", 0.5)])
    ...
    ///
    ///         >>> left = NoiseMapSam.from_list([("+", 0.5), ("-", 0.5)])
    ///         >>> right = NoiseMapSam.from_list([("r", 0.5), ("l", 0.5)])
    ...
    ///         :class:`NoiseMapSam` in the :class:`.SparsePauliOp` dense Pauli representation.
    ...
    ///
    /// This can be seen as counter-operation of :meth:`.NoiseMapSam.from_sparse_list`, however
    /// the order of terms is not guaranteed to be the same at after a roundtrip to a sparse
    /// list and back.
    ///
    /// Examples:
    ///
    ///     >>> obs = NoiseMapSam.from_list([("IIXIZ", 2j), ("IIZIX", 2j)])
    ///     >>> reconstructed = NoiseMapSam.from_sparse_list(obs.to_sparse_list(), obs.num_qubits)
    ///
    /// See also:
    ///     :meth:`from_sparse_list`
    ...
    fn to_sparse_list(&self, py: Python) -> PyResult<Py<PyList>> {
        ...
        let to_py_tuple = |view: SparseTermViewSam| {
    ...
    /// Construct a :class:`.NoiseMapSam` from a :class:`.SparsePauliOp` instance.
    ///
    ...
    ///         >>> NoiseMapSam.from_sparse_pauli_op(spo)
    ///         <NoiseMapSam with 3 terms on 3 qubits: (1+0j)() + (0.5+0j)(Z_0) + (0.5+0j)(Z_1)>
    #[staticmethod]
    #[pyo3(signature = (op, /))]
    fn from_sparse_pauli_op(op: &Bound<PyAny>) -> PyResult<Self> {
        ...
        let coeffs = op
            ...
            .map_err(|_| PyTypeError::new_err("only 'SparsePauliOp' with complex-typed coefficients can be converted to 'NoiseMapSam'"))?
            .as_array()
            .to_vec();
        ...
        let inner =
            NoiseMapSam::new(num_qubits as u32, coeffs, bit_terms, indices, boundaries)?;
        Ok(inner.into())
    }

    /// Construct a :class:`NoiseMapSam` out of individual terms.
    ///
    ...
    fn from_terms(obj: &Bound<PyAny>, num_qubits: Option<u32>) -> PyResult<Self> {
        ...
        let mut inner = match num_qubits {
            Some(num_qubits) => NoiseMapSam::zero(num_qubits),
            None => {
    ...
    // the `bit_terms` must all be valid `BitTermSam` representations.
    /// Construct a :class:`.NoiseMapSam` from raw Numpy arrays that match :ref:`the required
    ...
    ///             of :class:`.NoiseMapSam.BitTermSam`.  If they are not, Rust-space undefined
    ...
    ///
    ///         >>> num_qubits = 100
    ///         >>> terms = np.full((num_qubits,), NoiseMapSam.BitTermSam.Z, dtype=np.uint8)
    ...
    ///         >>> NoiseMapSam.from_raw_parts(num_qubits, coeffs, terms, indices, boundaries)
    ///         <NoiseMapSam with 100 terms on 100 qubits: (1+0j)(Z_0) + ... + (1+0j)(Z_99)>
    #[staticmethod]
    #[pyo3(
    ...
    unsafe fn from_raw_parts<'py>(
        ...
        let bit_terms = if check {
            bit_terms
                ...
                .into_iter()
                .copied()
                .map(BitTermSam::try_from)
                .collect::<Result<_, _>>()?
        } else {
            let bit_terms_as_u8 = bit_terms.as_array().to_vec();
            // SAFETY: the caller enforced that each `u8` is a valid `BitTermSam`, and `BitTermSam` is be
            // represented by a `u8`.  We can't use `bytemuck` because we're casting a `Vec`.
            unsafe { ::std::mem::transmute::<Vec<u8>, Vec<BitTermSam>>(bit_terms_as_u8) }
        };
        let indices = indices.as_array().to_vec();
        let boundaries = boundaries.as_array().to_vec();

        let inner = if check {
            NoiseMapSam::new(num_qubits, coeffs, bit_terms, indices, boundaries)
                .map_err(PyErr::from)
        } else {
            // SAFETY: the caller promised they have upheld the coherence guarantees.
            Ok(unsafe {
                NoiseMapSam::new_unchecked(num_qubits, coeffs, bit_terms, indices, boundaries)
            })
        }?;
        Ok(inner.into())
    }
    ...
    ///     .. code-block:: python
    ///
    ///         >>> obs = NoiseMapSam.from_list([("IX+-rl", 2.0), ("01YZII", -1j)])
    ///         >>> obs.clear()
    ///         >>> assert obs == NoiseMapSam.zero(obs.py_num_qubits())
    ...
    ///
    ///         >>> left = NoiseMapSam.from_list([("XYZ", 1.0/3.0)] * 3)   # sums to 1.0
    ///         >>> right = NoiseMapSam.from_list([("XYZ", 1.0/7.0)] * 7)  # doesn't sum to 1.0
    ///         >>> assert left.simplify() != right.simplify()
    ///         >>> assert (left - right).simplify() == NoiseMapSam.zero(left.num_qubits)
    #[pyo3(
        signature = (/, tol=1e-8),
    )]
    ...
    ///     .. code-block::
    ///
    ///         >>> left = NoiseMapSam.from_list([("XY+-", 1j)])
    ///         >>> right = NoiseMapSam.from_list([("XY+-", -1j)])
    ...
    ///     .. code-block::
    ///
    ///         >>> obs = NoiseMapSam([("III", 1j), ("Yrl", 0.5)])
    ///         >>> assert obs.transpose() == NoiseMapSam([("III", 1j), ("Ylr", -0.5)])
    ...
    ///
    ///         >>> obs = NoiseMapSam([("III", 1j), ("Yrl", 0.5)])
    ///         >>> assert obs.conjugate() == NoiseMapSam([("III", -1j), ("Ylr", -0.5)])
    ...
    ///
    /// The argument will be cast to :class:`NoiseMapSam` using its default constructor, if it
    ...
    ///
    ///         >>> left = NoiseMapSam.from_label("XYZ")
    ///         >>> right = NoiseMapSam.from_label("+-IIrl")
    ///         >>> assert left.tensor(right) == NoiseMapSam.from_label("XYZ+-IIrl")
    ///
    ///     You can also use the infix ``^`` operator for tensor products, which will similarly cast
    ///     the right-hand side of the operation if it is not already a :class:`NoiseMapSam`::
    ///
    ///         >>> assert NoiseMapSam("rl") ^ Pauli("XYZ") == NoiseMapSam("rlXYZ")
    ///
    /// See also:
    ///     :meth:`expand`
    ///
    ...
    ///         :class:`NoiseMapSam` to be on the right-hand side of the tensor ordering.
    ...
    /// to :class:`NoiseMapSam` if it isn't already one (by calling the default constructor).
    ///
    /// Args:
    ///
    ...
    #[pyo3(signature = (other, /))]
    fn expand<'py>(&self, other: &Bound<'py, PyAny>) -> PyResult<Bound<'py, PyNoiseMapSam>> {
        let py = other.py();
        let Some(other) = coerce_to_observable(other)? else {
            return Err(PyTypeError::new_err(format!(
    ...
    /// Compose another :class:`NoiseMapSam` onto this one.
    ///
    /// In terms of operator algebras, composition corresponds to left-multiplication:
    ...
    ///
    /// Beware that this function can cause exponential explosion of the memory usage of the
    /// observable, as the alphabet of :class:`NoiseMapSam` is not closed under composition;
    ...
    fn compose<'py>(
        ...
        front: bool,
    ) -> PyResult<Bound<'py, PyNoiseMapSam>> {
        let py = other.py();
        let Some(other) = coerce_to_observable(other)? else {
            return Err(PyTypeError::new_err(format!(
    ...
    /// Apply a transpiler layout to this :class:`NoiseMapSam`.
    ///
    ...
    ///
    /// Returns:
    ///     A new :class:`NoiseMapSam` with the provided layout applied.
    ...
    fn apply_layout(&self, layout: Bound<PyAny>, num_qubits: Option<u32>) -> PyResult<Self> {
        ...
        // input types, before calling NoiseMapSam.apply_layout to do the actual work.
    ...
    fn __getitem__<'py>(
        ...
        let mut out = NoiseMapSam::zero(inner.num_qubits());
    ...
    fn __repr__(&self) -> PyResult<String> {
        ...
        let str_terms = if num_terms == 0 {
            ...
            inner
                .iter()
                .map(SparseTermViewSam::to_sparse_str)
                .collect::<Vec<_>>()
                .join(" + ")
        };
        Ok(format!(
    ...
    fn __add__<'py>(
        ...
        if Arc::ptr_eq(&slf_.inner, &other.inner) {
            ...
            return <&NoiseMapSam as ::std::ops::Mul<_>>::mul(
                &inner,
        ...
        <&NoiseMapSam as ::std::ops::Add>::add(&slf_inner, &other_inner).into_bound_py_any(py)
    }
    ...
    fn __radd__<'py>(&self, other: &Bound<'py, PyAny>) -> PyResult<Bound<'py, PyAny>> {
        ...
        <&NoiseMapSam as ::std::ops::Add>::add(&other_inner, &inner).into_bound_py_any(py)
    }

    fn __iadd__(slf_: Bound<PyNoiseMapSam>, other: &Bound<PyAny>) -> PyResult<()> {
        let Some(other) = coerce_to_observable(other)? else {
            ...
            return Err(PyTypeError::new_err(format!(
                "invalid object for in-place addition of 'NoiseMapSam': {}",
                other.repr()?
            )));
        };
        ...
        // Check if slf_ and other point to the same NoiseMapSam object, in which case
    ...
    fn __sub__<'py>(
        ...
        let slf_ = slf_.borrow();
        if Arc::ptr_eq(&slf_.inner, &other.inner) {
            return PyNoiseMapSam::zero(slf_.num_qubits()?).into_bound_py_any(py);
        }

        let slf_inner = slf_.inner.read().map_err(|_| InnerReadError)?;
        let other_inner = other.inner.read().map_err(|_| InnerReadError)?;
        slf_inner.check_equal_qubits(&other_inner)?;
        <&NoiseMapSam as ::std::ops::Sub>::sub(&slf_inner, &other_inner).into_bound_py_any(py)
    }
    ...
    fn __rsub__<'py>(&self, other: &Bound<'py, PyAny>) -> PyResult<Bound<'py, PyAny>> {
        ...
        let other_inner = other.inner.read().map_err(|_| InnerReadError)?;
        inner.check_equal_qubits(&other_inner)?;
        <&NoiseMapSam as ::std::ops::Sub>::sub(&other_inner, &inner).into_bound_py_any(py)
    }

    fn __isub__(slf_: Bound<PyNoiseMapSam>, other: &Bound<PyAny>) -> PyResult<()> {
        let Some(other) = coerce_to_observable(other)? else {
            // This is not well behaved - we _should_ return `NotImplemented` to Python space
            // without an exception, but limitations in PyO3 prevent this at the moment.  See
            // https://github.com/PyO3/pyo3/issues/4605.
            return Err(PyTypeError::new_err(format!(
                "invalid object for in-place subtraction of 'NoiseMapSam': {}",
                other.repr()?
            )));
        };
        let other = other.borrow();
    ...
    fn __neg__(&self) -> PyResult<Self> {
        let inner = self.inner.read().map_err(|_| InnerReadError)?;
        let neg = <&NoiseMapSam as ::std::ops::Neg>::neg(&inner);
        Ok(neg.into())
    }

    fn __mul__(&self, other: Complex64) -> PyResult<Self> {
        let inner = self.inner.read().map_err(|_| InnerReadError)?;
        let mult = <&NoiseMapSam as ::std::ops::Mul<_>>::mul(&inner, other);
        Ok(mult.into())
    }
    ...
    fn __truediv__(&self, other: Complex64) -> PyResult<Self> {
        ...
        let inner = self.inner.read().map_err(|_| InnerReadError)?;
        let div = <&NoiseMapSam as ::std::ops::Div<_>>::div(&inner, other);
        Ok(div.into())
    }
    ...
    #[classattr]
    fn BitTermSam(py: Python) -> PyResult<Py<PyType>> {
        BIT_TERM_PY_ENUM
            .get_or_try_init(py, || make_py_bit_term(py))
...
impl From<NoiseMapSam> for PyNoiseMapSam {
    fn from(val: NoiseMapSam) -> PyNoiseMapSam {
        PyNoiseMapSam {
            inner: Arc::new(RwLock::new(val)),
        }
    }
}
impl<'py> IntoPyObject<'py> for NoiseMapSam {
    type Target = PyNoiseMapSam;
    type Output = Bound<'py, Self::Target>;
    type Error = PyErr;

    fn into_pyobject(self, py: Python<'py>) -> PyResult<Self::Output> {
        PyNoiseMapSam::from(self).into_pyobject(py)
    }
}

/// Helper class of `ArrayView` that denotes the slot of the `NoiseMapSam` we're looking at.
...
/// ability of `NoiseMapSam` to re-allocate in place; we can't leave dangling pointers for
/// Python space.
#[pyclass(frozen, sequence)]
struct ArrayView {
    base: Arc<RwLock<NoiseMapSam>>,
    slot: ArraySlot,
}
#[pymethods]
impl ArrayView {
    ...
    fn __getitem__<'py>(
        ...
        // different to the type that gets put into the Numpy array, since the `BitTermSam` enum can be
    ...
    fn __setitem__(&self, index: PySequenceIndex, values: &Bound<PyAny>) -> PyResult<()> {
        ...
        /// the error messages for things like `BitTermSam`, where Python-space extraction might fail
        ...
        match self.slot {
            ...
            ArraySlot::BitTerms => set_in_slice::<BitTermSam, u8>(obs.bit_terms_mut(), index, values),
    ...
    fn __array__<'py>(
        ...
        // `NoiseMapSam`, since the `Vec` we're referring to might re-allocate and invalidate
...
/// Attempt to coerce an arbitrary Python object to a [PyNoiseMapSam].
///
/// This returns:
///
...
) -> PyResult<Option<Bound<'py, PyNoiseMapSam>>> {
    let py = value.py();
    if let Ok(obs) = value.downcast_exact::<PyNoiseMapSam>() {
        return Ok(Some(obs.clone()));
    }
    match PyNoiseMapSam::py_new(value, None) {
...
pub fn noise_map(m: &Bound<PyModule>) -> PyResult<()> {
    m.add_class::<PyNoiseMapSam>()?;
    Ok(())
}
...
mod test {
    ...
    fn test_error_safe_add_dense_label() {
        let base = NoiseMapSam::identity(5);
